...
class PlayState extends FlxState {
	...
	override public function create() {
		...
		add(scoreText);
		insert(members.indexOf(scoreText),
			things[1] = new FlxSprite(0, FlxG.height - scoreText.height - 20).makeGraphic(800, Std.int(scoreText.height + 20), 0xff000000));
		for (i in things)
			i.alpha = 0.5;
	...
	override public function update(elapsed:Float) {
		...
		if (clickable
			&& circle.x < FlxG.mouse.x
			&& circle.x + circle.width > FlxG.mouse.x
			&& circle.y < FlxG.mouse.y
			&& circle.y + circle.height > FlxG.mouse.y
			&& FlxG.mouse.justPressed)
			handleClick();
		if (score >= 250) {
			if (rainbowModeText.x < FlxG.mouse.x
				&& rainbowModeText.x + rainbowModeText.width > FlxG.mouse.x
				&& rainbowModeText.y < FlxG.mouse.y
				&& rainbowModeText.y + rainbowModeText.height > FlxG.mouse.y
				&& FlxG.mouse.justPressed)
	...
	private function handleClick() {
		...
		if (score == 250)
			doRainbowMode();
		var motivationText:FlxText = new FlxText(0, 0, 0,
			motivationTexts[FlxG.random.int(0, motivationTexts.length - 1)]).setFormat(null, 24, 0xffffffff, "center");
		motivationText.setBorderStyle(OUTLINE, 0xff000000, 2);
		motivationText.scale.scale(1.1);
		motivationText.setPosition(FlxG.random.int(0, Std.int(FlxG.width - motivationText.width)),
			FlxG.random.int(Std.int(things[0].y + 50), Std.int(things[1].y - motivationText.height - 20)));
		textGroup.add(motivationText);
		var r = FlxG.random.float(1.1, 1.3);
		flixel.tweens.FlxTween.tween(motivationText, {
			alpha: 0,
			angle: FlxG.random.int(-20, 20),
			"scale.x": r,
			"scale.y": r
		}, 1, {
	...
	static final motivationTexts:Array<String> = [
		"yay!", "cool!", "you can do it!", "woohoo!", "nice!", "keep going!", "you're doing great!", "wow :)", "how amazing!", "oh joy", "circle!!!"
	];
}
